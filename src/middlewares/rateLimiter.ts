...
const rateLimiter =
    (secondsLimit: number, limitAmount: number, type: RateLimitType) =>
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        const email =
            ...
            (
                await UserService.viewSingleUserDynamic({
...
const slidingWindowRateLimiter =
    (windowSizeInSeconds: number, limitAmount: number, type: RateLimitType) =>
    async (req: Request, res: Response, next: NextFunction) => {
        // const ip = req.ip;
        // const userAgent = req.headers['user-agent'] as string;
        // const userLocation = (req.headers['x-forwarded-for'] || req.socket.remoteAddress) as string;
